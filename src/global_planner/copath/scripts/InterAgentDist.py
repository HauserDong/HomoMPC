...
class InterAgentDist():

    def __init__(self) -> None:

        rospy.init_node('inter_agent_dist', anonymous=False)

        self.puber=rospy.Publisher('/Commu',MarkerArray,queue_size=1)
        self.goal_sub = rospy.Subscriber('/goal1', PoseStamped, self.get_target_callback, queue_size=10)
    ...
    def get_target_callback(self, msg):
        if self.stop_recording:
            ...
            rospy.loginfo("Start recording inter-agent distance")
    ...
    def cal_inter_agent_dist(self):

        inter_dist_lst_tmp = []
        
        for ob1 in self.object_list:
            p1 = self.query_p(ob1.index)

            for ob2 in self.object_list:
                if ob2.index != ob1.index:
                    p2 = self.query_p(ob2.index)
                    
                    if p1 is not None and p2 is not None:
                        dist = np.linalg.norm(p1 - p2)

                        inter_dist_lst_tmp.append(dist)
        
        self.inter_dist_lst.append(inter_dist_lst_tmp)


    def check_all_reach_target(self):

        all_reach_target = True
        
        for ob in self.object_list:
            idx = ob.index
            # target = SET.agent_list[idx-1]['tar']
            target = SET.agent_list[idx]['tar']
            p = self.query_p(idx)
        ...
        self.all_reach_target = all_reach_target
    ...
    def run(self) -> None:
        ...
        if not self.all_reach_target and not self.stop_recording:
            self.cal_inter_agent_dist()

            self.check_all_reach_target()
        
        # print(self.all_reach_target)
        # print(self.stop_recording)
        if self.all_reach_target and not self.stop_recording:
            
            rospy.loginfo("Inter-agent distance saved")

            import pickle
            with open('inter_agent_dist_'+str(SET.test_mode)+'.pkl', 'wb') as f:
                pickle.dump(self.inter_dist_lst, f)
